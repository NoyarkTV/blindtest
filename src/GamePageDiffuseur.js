...
useEffect(() => {
  if (deviceId && playlist.length > 0 && currentRound === 1) {
    setTimeLeft(params.time);
    if (isDiffuser) playCurrentTrack(deviceId);
    handleNextRoundPopup();
  }
}, [deviceId, playlist, params]);

  useEffect(() => {
    if (deviceId && playlist.length > 0 && isDiffuser) {
      playCurrentTrack(deviceId);
    }
  }, [deviceId]);

useEffect(() => {
  if (deviceId && playlist.length > 0 && !isPlaying && !isBuzzed && isDiffuser) {
    playCurrentTrack(deviceId);
  }
}, [deviceId, playlist]);
...
useEffect(() => {
  ...
  return () => socket.off("all-ready", onAllReady);
}, [isDiffuser, currentRound, playlist]);
...
const handleBuzz = () => {
    ...
    if (isDiffuser) {
      ...
      socket.emit("pause-track", { roomId: id });
    } else {
      socket.emit("player-buzz", { roomId: id, playerName });
    }
};
...
// 🔴 Cas 3 : Mauvaise réponse
  ...
  const handleReady = (id) => {
    setDeviceId(id);
    if (isDiffuser) playCurrentTrack(id);
  };
...
export default GamePageDiffuseur;